...
var (
	...
	_ N.TCPConnectionHandlerEx = (*Weighted)(nil)
	_ N.UDPConnectionHandlerEx = (*Weighted)(nil)
)
...
// going through.
func (w *Weighted) NewConnectionEx(ctx context.Context, conn net.Conn, source M.Socksaddr, destination M.Socksaddr, onClose N.CloseHandlerFunc) {
...
func (w *Weighted) NewPacketConnectionEx(ctx context.Context, conn N.PacketConn, source M.Socksaddr, destination M.Socksaddr, onClose N.CloseHandlerFunc) {
	var metadata adapter.InboundContext
	metadata.Source = source
	metadata.Destination = destination
